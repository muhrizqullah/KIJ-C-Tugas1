...
class DiyAes(object):
    ...
    def __key_expansion(self):
        self.round_keys = self.__bytes_to_matrix(self.key)

        for i in range(4, 44):
            temp = self.round_keys[-1]

            if i % 4 == 0:
                temp = self.__rot_word(temp)
    ...
    def __sub_bytes(self, state):
        for i in range(len(state)):
            state[i] = [S_BOX[j] for j in state[i]]

    def __inverse_sub_bytes(self, state):
        for i in range(len(state)):
            state[i] = [INVERSE_S_BOX[j] for j in state[i]]

    def __mix_single_column(self, column):
        temp = column.copy()
        column[0] = MUL_2[temp[0]] ^ MUL_3[temp[1]] ^ temp[2] ^ temp[3]
        column[1] = temp[0] ^ MUL_2[temp[1]] ^ MUL_3[temp[2]] ^ temp[3]
        column[2] = temp[0] ^ temp[1] ^ MUL_2[temp[2]] ^ MUL_3[temp[3]]
        column[3] = MUL_3[temp[0]] ^ temp[1] ^ temp[2] ^ MUL_2[temp[3]]

    def __mix_columns(self, state):
        for i in range(len(state)):
            self.__mix_single_column(state[i])

    def __inverse_mix_single_column(self, column):
        temp = column.copy()
        column[0] = MUL_14[temp[0]] ^ MUL_11[temp[1]] ^ MUL_13[temp[2]] ^ MUL_9[temp[3]]
        column[1] = MUL_9[temp[0]] ^ MUL_14[temp[1]] ^ MUL_11[temp[2]] ^ MUL_13[temp[3]]
        column[2] = MUL_13[temp[0]] ^ MUL_9[temp[1]] ^ MUL_14[temp[2]] ^ MUL_11[temp[3]]
        column[3] = MUL_11[temp[0]] ^ MUL_13[temp[1]] ^ MUL_9[temp[2]] ^ MUL_14[temp[3]]

    def __inverse_mix_columns(self, state):
        for i in range(len(state)):
            self.__inverse_mix_single_column(state[i])
    ...
    def __inverse_shift_rows(self, state):
    ...
    def __encrypt_block(self, data):
        ...
        for i in range(1, 10):
            ...
            self.__add_round_key(state, self.round_keys[i*4:(i+1)*4])
    ...
    def __decrypt_block(self, data):
        ...
        self.__add_round_key(state, self.round_keys[40:])
        self.__inverse_shift_rows(state)
        self.__inverse_sub_bytes(state)

        for i in range(9, 0, -1):
            self.__add_round_key(state, self.round_keys[i*4:(i+1)*4])
            self.__inverse_mix_columns(state)
            self.__inverse_shift_rows(state)
            self.__inverse_sub_bytes(state)
